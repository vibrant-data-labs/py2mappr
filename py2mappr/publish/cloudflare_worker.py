...
def cloudflare_worker(cdn_url: str, url: str):
    ...
    domain = list(filter(lambda zone: zone.get('name') ==
                  zone_name, cf_zone.json().get('result')))
    if (len(domain) == 0):
    ...
    requests.post('https://api.cloudflare.com/client/v4/zones/%s/dns_records' %
                  domain_item.get('id'), headers=auth_header, json=data)
